...
import { WarningOutlined, BellOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
import {
  BarChart,
  Bar,
  XAxis,
...
import BoLocLichSu from '@/components/history/BoLocLichSu';
import BangLichSu from '@/components/history/BangLichSu';
import ChiTietLichSuCard from '@/components/history/ChiTietLichSuCard';
import DrawerBieuMauLichSu from '@/components/history/DrawerBieuMauLichSu';
...
import type { LoaiTabHienThi } from '@/components/history/BangLichSu';
...
export default function HistoryPage() {
  ...
  // Trích xuất danh sách lịch sử và các hành động từ hook
  const { danhSachLichSu, dangTai, taiLai } = dungLichSu();

  // Trích xuất bản ghi đang chọn và hàm cập nhật từ Zustand store
  const { lichSuDuocChon, datLichSuDuocChon, datMoDrawerBieuMau } = dungKhoLichSu();
  ...
  }, [danhSachLichSu, lichSuDuocChon, datLichSuDuocChon]);
  ...
  const xuLyPhanHoiSaiThongTin = (lichSu: LichSuRaVao) => {
    Modal.confirm({
      ...
      content: `Bạn muốn báo cáo sai lệch thông tin của ${lichSu.hoVaTen} (${lichSu.mssv})?`,
      okText: 'Gửi phản hồi',
      cancelText: 'Hủy bỏ',
      onOk: () => {
  ...
  const danhSachTabs: { key: LoaiTabChinh; nhan: string }[] = [
    { key: 'LICH_SU', nhan: 'Lịch sử' },
    { key: 'THONG_KE', nhan: 'Thống kê' },
    { key: 'CO_MAT', nhan: 'Sinh viên đang có mặt' },
    { key: 'VANG_MAT', nhan: 'Sinh viên vắng mặt' },
    { key: 'CANH_BAO', nhan: 'Cảnh báo' },
  ];
  ...
  return (
    <BoCucChinh>
      ...
      <div className="mb-5 bg-zinc-100/80 p-1 rounded-xl inline-flex flex-wrap items-center gap-1 border border-zinc-200/50 shadow-sm">
        {danhSachTabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer border-none ${
                laActive
                  ? 'bg-white text-[#00afef] shadow-sm font-bold scale-[1.02]'
          ...
          <div className="flex-1 bg-white rounded-xl border border-zinc-200 p-5 shadow-sm min-w-0">
            ...
            <div className="flex items-center gap-3 mb-4 border-b border-zinc-100 pb-3">
              <button
                ...
                className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-all duration-150 cursor-pointer ${
